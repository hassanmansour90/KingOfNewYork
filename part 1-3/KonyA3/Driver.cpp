	...
	*/
	...
	while (keepPlaying == true)
	{
		...
		for (pos; pos < NUMPLAYERS; pos++)
		{
			...
			if (playervector[pos]->getActivity() == true)
			{
				...
				if (playervector[pos]->getFirstTurn() == true)
					playervector[pos]->setFirstTurn(false);
				...
				vector<bool> lowManVisited = myMap.getGraph().manhattanAvailable();
				if (!lowManVisited.at(0)) {
				playervector[pos]->moveToManhattan("Lower-Zone|2-4");
				cout << endl << playervector[pos]->getMonsterName() << " has been placed in Lower Manhattan (2-4)" << endl;
				}
				else if (!lowManVisited.at(1) && playervector.size() > 4) {
				playervector[pos]->moveToManhattan("Lower-Zone|5-6");
				cout << endl << playervector[pos]->getMonsterName() << " has been placed in Lower Manhattan (5-6)" << endl;
				}
				else {
				cout << endl << playervector[pos]->getMonsterName() << ", would you like to move?" << endl;
				...
				//ASK PLAYER IF THEY WANT TO BUYCARDS
				do {
				cout << endl << playervector[pos]->getMonsterName() << ", would you like to buy cards?" << endl;
				cout << endl << "Enter Y or N" << endl << endl;
				cin >> input;
				while (!(input == 'y') && !(input == 'Y') && !(input == 'n') && !(input == 'N'))
				{
				...
				{
				if (playervector[k]->getActivity() == true)
				playervector[k]->displayMonsterStats();
				}
				keepPlaying = false;
				}
				}
