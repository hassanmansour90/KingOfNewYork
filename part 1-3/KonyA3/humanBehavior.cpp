					...
					for (int i = 0; i < plVect.size(); i++)
					{
						if (plVect[i]->getRegion().getName().find("Zone") == string::npos)
						{
							if (plVect[i]->getRegion().getName() != "Manhattan")
							{
								...
								////////////////////////////////////////////////////////////////////////////////////////////
								for (int i = 0; i < plVect[i]->cards.size(); i++)
								{
									string aCard = plVect[i]->cards.at(i).getCardName();
									if (aCard == "Drain")
									{
								...
								////////////////////////////////////////////////////////////////////////////////////////////
								points = points - (attackCount + cardEffect);////////////////////////////////////////////////////////////////
		...
		case 'o': case 'O':
			...
			if ((p->getRegion().getUnits().size() != 0))
			{
				...
				if (ouchCount == 3)
				{
					...
					int damage = 0;
					for (int j = 0; j < myMap->getGraph().regions.at(j).getUnits().size(); j++)
					{
						damage = damage + myMap->getGraph().regions.at(j).getUnits().size();
					}
					int life;
					for (int i = 0; i < plVect.size(); i++)
					{
		...
		if ((input == 'y') || (input == 'Y'))
		{
			...
			{ // a player choose a card
				...
				while (!haveEnoughEnergyPoints)
				{
					...
					cin >> cardNumber;
					haveEnoughEnergyPoints = (p->getEnergyPoints() > myMap->getCards().at(cardNumber - 1).getCost());
				}
				if (counttt != 3) {
					p->setEnergyPoints(p->getEnergyPoints() - (myMap->getCards().at(cardNumber - 1).getCost()));
					p->getCards().push_back(myMap->getCards().at(cardNumber - 1));
					vector<Cards> newC;
					for (int m = 0; m < myMap->getCards().size(); m++) {
						if (m != cardNumber - 1)
							newC.push_back(myMap->getCards().at(m));
					}
					cout << "\nhere is your new card you got: " << (myMap->getCards().at(cardNumber - 1).getCardName()) << endl;
