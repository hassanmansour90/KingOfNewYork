...
void moderateBehavior::chooseMonster(Map* myMap, Player* p)
{
	...
	int value;
	string name;
	size_t size = myMap->monstersOptions.size();
	if (size == 6)
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			cout << endl << i + 1 << ". " << myMap->monstersOptions[i];
		}
		cout << endl;
		...
		cout << endl << "You chose " << myMap->monstersOptions[value - 1] << "!" << endl;
		name = myMap->monstersOptions[value - 1];
		...
		myMap->monstersOptions.erase(myMap->monstersOptions.begin() + (value - 1));
	...
	if (size == 1)
	{
		cout << endl << "You will be " << myMap->monstersOptions[0] << "!" << endl;
		name = myMap->monstersOptions[0];
		for (int i = 0; i < mnstr.size(); i++)
		{
			if (name == mnstr[i]->getMonsName())
				p->setMonster(mnstr[i]);
			...
			if (p->getFirstTurn() != true)
			{
				...
				if (p->getRegion().getName().find("Zone") != string::npos)
				{
					...
					for (int i = 0; i < plVect.size(); i++)
					{
						...
						if (plcount == 0)
						{
							cout << endl << "There are no other players currently outside of Manhattan.";
